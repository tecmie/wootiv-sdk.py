...
class TestRegistry:
    ...
    def test_method_retrieve(self, client: Tecmie) -> None:
        registry = client.mailbox.stream.registry.retrieve(
            "string",
            id="string",
        )
        assert_matches_type(PaginatedStreamRegistry, registry, path=["response"])
...
class TestAsyncRegistry:
    ...
    async def test_method_retrieve(self, client: AsyncTecmie) -> None:
        registry = await client.mailbox.stream.registry.retrieve(
            "string",
            id="string",
        )
        assert_matches_type(PaginatedStreamRegistry, registry, path=["response"])
